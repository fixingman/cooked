...
- `0.28.2` fix: Cookidoo + NYT Cooking import — URL-tab shows paste-mode guidance; Cookidoo paste preserves TM step notation (`| Varoma | Speed 1`) via `sourceHint` in `extractWithClaude`; detected by URL or inline TM pattern; NYT hint strips step labels + bullet chars
...
- `0.29.2` feat: shopping list grouped by category (`inferCategory` reuse); per-item "running low" label removed
